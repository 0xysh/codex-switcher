  ...
  return (
    <section className="surface-panel reveal-rise stagger-2 border-[var(--border-strong)] p-5 sm:p-6" aria-label="Current Codex Session">
      <div className="flex items-start justify-between gap-4">
        ...
        <IconButton
          aria-label={isCollapsed ? "Expand current session panel" : "Collapse current session panel"}
          aria-expanded={!isCollapsed}
          aria-controls="current-session-panel-body"
          size="md"
      ...
      {!isCollapsed ? (
        <div id="current-session-panel-body">
          <dl className="mt-4 grid gap-2 rounded-2xl border border-[var(--border-soft)] bg-[var(--bg-surface)] p-3 text-xs text-secondary shadow-[var(--shadow-soft)] xl:ml-auto xl:max-w-xl">
